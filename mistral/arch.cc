...
CycloneV::rnode_index Arch::find_rnode(CycloneV::block_type_t bt, int x, int y, CycloneV::port_type_t port, int bi,
                                   int pi) const
{
...
{
    CycloneV::rnode_index rn = find_rnode(bt, x, y, port, bi, pi);
    if (rn)
        return WireId(cyclonev->ri2rc(rn));
...
Arch::Arch(ArchArgs args)
{
    ...
    int pip_count = 0;
    for (const CycloneV::rnode_object* rnode = cyclonev->ro_begin; rnode != cyclonev->ro_end; rnode = rnode->next()) {
        WireId dst_wire(rnode->rc());
        for (uint32_t srcIdx = 0; srcIdx < rnode->sources_count(); srcIdx++) {
            CycloneV::rnode_index src = rnode->sources_begin()[srcIdx];
            WireId src_wire(cyclonev->ri2rc(src));
